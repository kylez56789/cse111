...
void object::move (GLfloat delta_x, GLfloat delta_y, 
   ...
   if (center.xpos + delta_x > window_width) {
      center.xpos = 0;
   }
   else if (center.xpos + delta_x < 0) {
      center.xpos = window_width;
   }
   else {
      center.xpos += delta_x;
   }
   if (center.ypos + delta_y > window_height) {
      center.ypos = 0;
   }
   else if(center.ypos + delta_y < 0) {
      center.ypos = window_height;
   }
   else {
      center.ypos += delta_y;
   }
...
void window::display() {
   ...
   for (auto& object: window::objects) {
      object.draw();
      if(obj_num == selected_obj) {
         //object.draw_border();
      }
      ver = object.get_center();
      x_pos = ver.xpos;
      y_pos = ver.ypos;
      string print = "(" + to_string(x_pos) + "," + to_string(y_pos) + ")";
      void* font = GLUT_BITMAP_TIMES_ROMAN_10;
      auto u_str = reinterpret_cast<const GLubyte*>(print.c_str());
      const GLubyte black[3] = {0,0,0};
      glColor3ubv (black);
      glRasterPos2f(x_pos + 40,y_pos);
      glutBitmapString(font,u_str);
      
      const GLubyte red[3] = {255,0,0};
      string num_str = to_string(obj_num);
      glColor3ubv(red);
      auto new_str = reinterpret_cast<const GLubyte*>(num_str.c_str());
      glRasterPos2f(x_pos,y_pos);
      glutBitmapString(font,new_str);
   }
   mus.draw();
   glutSwapBuffers();
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         objects[selected_obj].move(-move_pixels,0, width, height);
         //move_selected_object (
         break;
      case 'J': case 'j':
         objects[selected_obj].move(0,-move_pixels, width, height);
         //move_selected_object (
         break;
      case 'K': case 'k':
         objects[selected_obj].move(0,move_pixels, width, height);
      ...
      case 'N': case 'n': case SPACE: case TAB:
         if(selected_obj + 1 >= objects.size()) {
            selected_obj = 0;//If at end, go to beginning
         }
         else {
            ++selected_obj;
         }
      ...
      case 'P': case 'p': case BS:
         if(selected_obj - 1 <= 0) {
            selected_obj = objects.size() - 1;//If at beginning, go to end
         }
         else {
            --selected_obj;
         }
